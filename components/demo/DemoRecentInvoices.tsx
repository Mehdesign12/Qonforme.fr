...
const STATUS_DOT: Record<InvoiceStatus, string> = {
  draft: "#94A3B8", sent: "#3B82F6", pending: "#D97706", received: "#8B5CF6",
  accepted: "#10B981", rejected: "#EF4444", paid: "#10B981", overdue: "#EF4444",
  cancelled: "#94A3B8", credited: "#F97316",
}
  ...
  return (
    <div className="rounded-2xl border border-white/60 overflow-hidden"
          ...
          <div key={inv.id} className="flex items-center gap-3 px-4 py-3.5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${STATUS_STYLE[inv.status]}`}>
                  <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: STATUS_DOT[inv.status] }} />
                  {INVOICE_STATUS_LABELS[inv.status]}
                </span>
              </div>
              <p className="text-[12px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{inv.client}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="font-mono text-[13px] font-bold text-[#0F172A] dark:text-[#E2E8F0]">{formatCurrency(inv.total_ttc)}</p>
              <p className="text-[10px] text-slate-300 dark:text-slate-500 mt-0.5">{formatDate(inv.issue_date)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <table className="hidden sm:table w-full">
        <thead>
          <tr className="border-b border-[#F8FAFC] dark:border-[#162032] bg-[#FAFBFC]/60 dark:bg-[#162032]/60">
            <th className="text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-500 px-5 py-3">N° facture</th>
            <th className="text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-500 px-5 py-3">Client</th>
            <th className="text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-500 px-5 py-3 hidden md:table-cell">Date</th>
            <th className="text-right text-[10px] font-bold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-500 px-5 py-3">Montant TTC</th>
            <th className="text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-500 px-5 py-3">Statut</th>
          </tr>
        </thead>
        <tbody>
          {MOCK_INVOICES.map((inv, i) => (
            <tr key={inv.id}
              className="border-b border-[#F8FAFC] dark:border-[#162032] hover:bg-[#F8FAFC]/60 dark:hover:bg-[#162032]/60 transition-colors last:border-0">
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center gap-2.5">
                  <ClientAvatar name={inv.client} index={i} />
                  <span className="text-[13px] font-medium text-[#0F172A] dark:text-[#E2E8F0] truncate">{inv.client}</span>
                </div>
              </td>
              ...
              <td className="px-5 py-3.5">
                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${STATUS_STYLE[inv.status]}`}>
                  <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: STATUS_DOT[inv.status] }} />
                  {INVOICE_STATUS_LABELS[inv.status]}
                </span>
              </td>
            </tr>
      ...
      {/* Footer */}
      <div className="px-5 py-3 border-t border-[#F8FAFC] dark:border-[#162032] flex items-center justify-between bg-[#FAFBFC]/40 dark:bg-[#162032]/40">
        <Link href="/demo/invoices"
