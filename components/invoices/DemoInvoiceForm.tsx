...
import { Plus, Trash2, Eye, Send, Lock } from "lucide-react"
          ...
          <div key={field.id} className="grid grid-cols-12 gap-2 items-start">
            ...
            {(Number(lines[index]?.quantity) > 0 && Number(lines[index]?.unit_price_ht) > 0) && (
              <div className="col-span-12 flex justify-end text-xs text-slate-400">
                HT : <span className="font-mono font-medium text-[#0F172A] mx-1">{formatCurrency(computedLines[index]?.totalHT ?? 0)}</span>
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-medium">{formatCurrency(totals.subtotal_ht)}</span>
            </div>
            <div className="flex justify-between">
