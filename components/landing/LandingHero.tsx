...
function Header() {
  ...
  return (
    <div className="sticky top-0 z-50 flex justify-center px-5 pt-4">
      <motion.nav
        /* Transition rectangle → pilule au scroll */
        animate={{
        ...
        className="flex w-full max-w-5xl items-center justify-between border border-white/70 py-2.5 backdrop-blur-md"
      >
        {/* ── Logo ── */}
        ...
        <div className="hidden shrink-0 items-center gap-2 md:flex">
          <Link href="/login">
            <Button
              variant="ghost"
              className="h-8 px-3 text-sm font-medium text-slate-500 hover:text-[#0F172A]"
            >
              Se connecter
            </Button>
          </Link>
            ...
            <nav className="flex flex-col gap-3">
              {navLinks.map((l) => (
                <Link key={l.label} href={l.href} onClick={() => setMobileOpen(false)}
                  className="rounded-lg px-3 py-2 text-sm text-slate-500 transition-colors hover:bg-slate-50 hover:text-[#0F172A]">
                  {l.label}
                </Link>
              ))}
              ...
              <Link href="/signup" onClick={() => setMobileOpen(false)}>
                <ShimmerButton background="rgba(37,99,235,1)" shimmerColor="#ffffff" shimmerDuration="2.5s" borderRadius="9999px" className="w-full h-9 text-sm justify-center">
                  Essai gratuit
                </ShimmerButton>
              </Link>
            </nav>
...
function Hero() {
  return (
    <section className="mx-auto max-w-6xl px-5 pb-20 pt-10 sm:pt-12">
      <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-[1fr_1.05fr] lg:gap-16">
        ...
        >
          ...
          >
            ...
            Transmission légale automatique, archivage 10 ans, <strong className="font-semibold text-[#0F172A]">zéro jargon.</strong>
          </motion.p>
        ...
        >
          ...
          <div className="relative overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white shadow-[0_20px_60px_-10px_rgba(15,23,42,0.15)]">
            ...
            <div className="flex items-center gap-1.5 border-b border-[#F1F5F9] bg-[#F8FAFC] px-4 py-2.5">
              ...
              <span className="mx-auto font-mono text-[11px] text-slate-400 tracking-wide">
            ...
            <div className="bg-[#F8FAFC] p-4">
              ...
              <div className="mb-3 grid grid-cols-2 gap-2.5">
                {[
                  { label: "CA ce mois", value: "8 450 €", delta: "+12 %", deltaColor: "#059669" },
                  { label: "Factures émises", value: "7", delta: "ce mois", deltaColor: "#2563EB" },
                  { label: "En attente", value: "2 100 €", delta: "3 factures", deltaColor: "#D97706" },
                  { label: "En retard", value: "0 €", delta: "tout est ok ✓", deltaColor: "#059669" },
                ].map((kpi) => (
                  <div key={kpi.label} className="rounded-xl border border-[#E8EFF6] bg-white px-3 py-2.5">
                    ...
                    <p className="text-[11px] mt-0.5 font-medium" style={{ color: kpi.deltaColor }}>{kpi.delta}</p>
                  </div>
                ))}
              </div>
                ...
                {[
                  ...
                  <div key={row.num} className="flex items-center justify-between border-b border-[#F8FAFC] px-3.5 py-2.5 last:border-0">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="rounded-full px-2 py-0.5 text-[11px] font-semibold" style={{ color: row.sc, backgroundColor: row.sb }}>
                        {row.status}
                      </span>
                    </div>
                  </div>
